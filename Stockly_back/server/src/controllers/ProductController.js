...
exports.createProduct = async (req, res) => {
  ...
  try {
    ...
    let compteur = (await Produit.count()) + 1; 
    let codeBarre;
    let produitExistant;

    do {
      const compteurStr = String(compteur).padStart(5, "0");
      codeBarre = `PRD-${yyyyMMdd}-${compteurStr}`;
      produitExistant = await Produit.findOne({ where: { codeBarre } });
      compteur++;
    } while (produitExistant);
    ...
    for (let produit of produits) {
      ...
      try {
        ...
        return res.status(500).json({
          message: `Erreur lors de la mise à jour du produit avec l'ID ${id}: ${error.message}`,
        });
      }
    }
